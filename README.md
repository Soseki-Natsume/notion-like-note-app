...
## ! 操作をお試しいただくうえでの注意点
RenderというPaaSの無料プランを利用しているためいくつかの制約があります。  
- DBを永続化していないため、トランザクション有無にかかわらず、サーバー再起動のたびにデータは失われます。
- 初期起動時、少し読み込みに時間がかかります。定期的にPingやなんらかのリクエストを送信すれば防げる問題ではあるのですが、あまり褒められた使用方法ではなさそうなので実施しません。ご理解いただけると幸いです。
...
- バックエンド
  - 言語: Go
  - API: 標準ライブラリ
  - 外部ライブラリ: [github.com/mattn/go-sqlite3](https://github.com/mattn/go-sqlite3), [github.com/google/uuid](https://github.com/google/uuid)
  - DB: SQLite3
- デプロイ
  - Dockerfile
  - Render(PaaS)
...
平たく言えば非同期なWebアプリケーションの作り方を学ぶことができました。
